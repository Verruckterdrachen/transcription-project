#!/usr/bin/env python3
"""
corrections/boundary_fixer.py - Boundary correction v16.42
...
def is_journalist_phrase(text):
    """
    🆕 v16.42: FIX БАГ #13 - Паттерн \bвопрос (БЕЗ \b в конце!)
    
    **ПРОБЛЕМА v16.41:**
    Pattern r'\bвопрос\b' требовал ТОЧНОЕ слово "вопрос"
    "вопроса", "вопросов", "Два вопроса" НЕ детектировались!
    
    **FIX v16.42:**
    Pattern r'\bвопрос' - начало слова "вопрос"
    Покрывает все формы: вопрос, вопроса, вопросов, вопросами
    ...
    journalist_markers = [
        r'\bвопрос',  # 🆕 v16.42: FIX БАГ #13 (убран \b в конце!)
...
def split_mixed_speaker_segments(segments_merged, speaker_surname, speaker_roles, debug=True):
    ...
    """
    print("\n✂️ Разделение mixed-speaker сегментов (v16.42: FIX journalist detection)...")
