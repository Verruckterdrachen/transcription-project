...
def clean_loops(text, debug=False):
    ...
    **FIX v17.9:**
    ...
    while i < len(words):
        ...
        if is_loop:
            # 🆕 v17.8 DEBUG BAG_A: логировать контекст удаления
            if debug:
                ...
                print(f"         Слово перед удалением: '{cleaned[-1] if cleaned else \"(начало)\"}'")
                # Проверяем: не останется ли обрубок (предлог без продолжения)
                HANGING_PREPOSITIONS = {'на', 'в', 'во', 'с', 'со', 'к', 'по', 'из', 'за', 'до',
                                        'при', 'через', 'о', 'об', 'у', 'для', 'от', 'под', 'над'}
                last_word = cleaned[-1].lower().rstrip('.,!?') if cleaned else ''
                if last_word in HANGING_PREPOSITIONS:
                    print(f"         🔴 РИСК ОБРУБКА! Последнее слово '{cleaned[-1]}' — предлог без продолжения!")
            i += 1
            continue
