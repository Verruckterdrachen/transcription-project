#!/usr/bin/env python3
"""
transcribe_v16.py - Главный файл пайплайна транскрибации v16.23

🔥 v16.23: FIX БАГ #3 - Hallucination loops в gap-filled сегментах
- Новая функция clean_loops() с n-gram анализом (размер 2-8 слов)
- Adaptive threshold: короткие фразы (<5 слов) → 85%, длинные → 75%
- Gap-filled сегменты: +10% к threshold (более строгая проверка)
- Проверка минимального расстояния между n-граммами (избегает overlapping)
- Результат: TXT без hallucination loops ("учитывать... учитывать... учитывать")

🔥 v16.22: FIX БАГ #1, #2, #3: Timestamp дубли + назад + Loop artifacts
...
- БАГ #3: clean_loops() использует fuzzy matching для детекции вариаций слов
- Результат: TXT без дублей timestamp, без обратного хода времени, без loop artifacts
...
VERSION = "16.23"
VERSION_NAME = "Fix БАГ #3 - Hallucination loops в gap-filled сегментах"
...
def main():
    ...
    print("=" * 70)
    print()
    print("💡 v16.23 ИЗМЕНЕНИЯ:")
    print("   ✅ БАГ #3 FIX: Hallucination loops в gap-filled сегментах")
    print("   ✅ clean_loops() с n-gram анализом (размер 2-8 слов)")
    print("   ✅ Adaptive threshold: короткие <5 слов → 85%, длинные → 75%")
    print("   ✅ Gap-filled сегменты: +10% к threshold (более строгий)")
    print("   ✅ Проверка min distance (избегает overlapping false positives)")
    print()
    ...
    print("   ✅ clean_loops: fuzzy matching (similarity ≥75%)")
    print()
