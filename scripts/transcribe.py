#!/usr/bin/env python3
"""
transcribe_v16.py - Главный файл пайплайна транскрибации v16.40

🔥 v16.40: FIX БАГ #11 - Изменение порядка: SPLIT → TIMESTAMP INJECTION
- Timestamp injection ПОСЛЕ split (не до!)
- Timestamp ВСЕГДА на границе предложений (100% с точкой)
- Убран workaround в is_continuation_phrase()
- Упрощена логика timestamp injection (не нужен поиск точки в радиусе 100 символов)

ROOT CAUSE v16.39:
- Timestamp injection ДО split → вставка между словами (8/19 без точки)
- Timestamp в начале текста ломает continuation patterns (r'^несмотря\b')

РЕШЕНИЕ v16.40:
- СНАЧАЛА split на предложения → точки известны
- ПОТОМ timestamp injection → вставка МЕЖДУ предложениями
...
from corrections.diarization_correction import correct_diarization_errors
...
# ═══════════════════════════════════════════════════════════════════════════
# 🆕 v16.28: DEBUG CHECKPOINT HELPER
# ═══════════════════════════════════════════════════════════════════════════

def debug_checkpoint(segments, stage_name, target_timestamps=None):
    """
    🆕 v16.28.4: Показ ПОЛНОГО текста сегмента для целевых timestamp
    """
    if target_timestamps is None:
        target_timestamps = ["00:04:29", "00:10:44"]
    ...
    for target_ts in target_timestamps:
        ...
        for seg in segments:
            ...
            if seg_start <= target_ts <= seg_end or seg_start == target_ts:
                ...
                # Целевые фразы
                target_phrases = {
                    "00:04:29": "то есть это был такой пункт",
                    "00:10:44": "то есть накопленный опыт"
                }
                
                phrase = target_phrases.get(target_ts, "").lower()
                has_phrase = phrase in text.lower() if phrase else None
                
                print(f"   ✅ {target_ts}: НАЙДЕН в [{seg_start}-{seg_end}] (длит={duration:.1f}s, слов={len(text.split())})")
                
                # 🆕 v16.28.4: Показываем ВЕСЬ текст для целевых timestamp
                if target_ts in ["00:04:29", "00:10:44"]:
                ...
                if phrase:
                    if has_phrase:
                        print(f"      ✅ Целевая фраза \"{phrase}\" НАЙДЕНА в тексте!")
                    else:
                        print(f"      ❌ Целевая фраза \"{phrase}\" НЕ НАЙДЕНА в тексте!")
...
VERSION = "16.40"
VERSION_NAME = "FIX БАГ #11: SPLIT → TIMESTAMP INJECTION (порядок этапов)"
...
def process_audio_file(
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # 🔥 v16.40: ЭТАП 8 - SPLIT MIXED-SPEAKER (ПЕРЕД timestamp injection!)
    # ═══════════════════════════════════════════════════════════════════════
    print("\n✂️ Разделение mixed-speaker сегментов (v16.40)...")
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # 🔥 v16.40: ЭТАП 8.1 - TIMESTAMP INJECTION (ПОСЛЕ split!)
    # Теперь timestamp вставляется МЕЖДУ предложениями, а не внутри длинного блока
    # ═══════════════════════════════════════════════════════════════════════
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # ЭТАП 8.2: TEXT-BASED CORRECTIONS (было 8.0)
    # ═══════════════════════════════════════════════════════════════════════
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # ЭТАП 8.3: HALLUCINATION REMOVAL (было 8.1)
    # ═══════════════════════════════════════════════════════════════════════
    ...
    # 🆕 v16.28: ФИНАЛЬНЫЙ CHECKPOINT перед экспортом
...
def main():
	...
	print("=" * 70)
	print()
	print("💡 v16.40 ИЗМЕНЕНИЯ:")
	print("   ✅ БАГ #11 FIX: Изменён порядок этапов: SPLIT → TIMESTAMP INJECTION")
	print("   ✅ Timestamp ВСЕГДА на границе предложений (100% с точкой)")
	print("   ✅ Убран workaround в is_continuation_phrase() (re.sub timestamp)")
	print("   ✅ Упрощена логика insert_intermediate_timestamps()")
	print("   ✅ ROOT CAUSE: Timestamp injection ДО split → вставка между словами")
	print("   ✅ РЕШЕНИЕ: Split ПЕРВЫМ → точки известны → timestamp МЕЖДУ предложениями")
	print()
