...
def test_raw_speaker_id_mapping():
    ...
    """
    # Подготовка: mixed сегмент с speaker="Исаев"
    segments_merged = [
        {
            "speaker": "Исаев",
            "text": "Первое предложение. Это журналист спрашивает. Третье предложение.",
            "start": 100.0,
            "end": 110.0,
            "time": "00:01:40",
    ...
    assert len(result) == 3, f"Ожидалось 3 сегмента, получено {len(result)}"
    
    # Первый и третий сегменты = "Исаев"
    assert result[0]['speaker'] == "Исаев"
    assert result[2]['speaker'] == "Исаев"
