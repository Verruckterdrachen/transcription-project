# 🐛 DEBUGGING.md — стратегия отладки v17.0

Документ описывает, как искать и устранять баги в транскрибационном пайплайне v17.0.

Основная философия:

- **Real tests > Integration > Unit**
- **Debug output + 5 Whys → ROOT CAUSE → FIX → Real test**
...
3. Включи/добавь **debug checkpoints** (см. ниже).

4. Сравни **checkpoint “ДО” и “ПОСЛЕ”**:
   - ищи, на каком этапе изменился текст/спикер/timestamp;
   - зафиксируй точный этап (например, «Этап 6.1 — Timestamp Injection»).

5. Прочитай код функций этого этапа:
   - найди место, где данные портятся;
   - сформулируй ROOT CAUSE через 5 Whys.

6. Придумай и реализуй FIX:
...
8. Обнови документацию:
   ...
   - при необходимости — `DEBUGGING.md`, `ARCHITECTURE.md`, `TESTING-STRATEGY.md`.
...
Урок БАГ #3:
Половина времени ушла на то, чтобы понять, что текст обрезается именно на этапе insert_intermediate_timestamps() — до этого не было системных checkpoint’ов.
...
Пример для БАГ #3 (усечённо):

Почему пропала фраза?
→ insert_intermediate_timestamps() обрезала последнее предложение.

Почему обрезала?
→ range(0, len(sentences)-1, 2) пропускал последний элемент массива.
...
ROOT CAUSE:
→ Ошибка в утилите парсинга + отсутствие unit‑теста на неё.
...
После фикса:
 ...
 При необходимости обновлены DEBUGGING.md, ARCHITECTURE.md, TESTING-STRATEGY.md
