...
**Правило:** разные типы файлов отправляются по-разному.
...
**Действие:** Копируешь содержимое → заменяешь файл → сохраняешь.

#### Файлы, в которые AI присылает ТОЛЬКО НОВЫЙ БЛОК:

- ✅ `docs/CHANGELOG.md` — добавляется блок `[vX.X]` в **самое начало** файла
